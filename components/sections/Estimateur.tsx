...
// ⚙️  Set your Mapbox public token in .env.local:  NEXT_PUBLIC_MAPBOX_TOKEN=pk.ey...
...
export default function Estimateur() {
  ...
  const calculate = async () => {
    ...
    try {
      ...
      setResult({ price: `CHF ${prix}`, detail: `${distKm} km · ${durMin} min de trajet estimé`, from: departVal, to: arriveeVal })
      ...
      if (mapbox) {
        const draw = () => {
          if (mapbox.getSource('route')) {
            mapbox.getSource('route').setData({ type: 'Feature', geometry: route.geometry })
          } else {
            mapbox.addSource('route', { type: 'geojson', data: { type: 'Feature', geometry: route.geometry } })
            mapbox.addLayer({ id: 'route', type: 'line', source: 'route', layout: { 'line-join': 'round', 'line-cap': 'round' }, paint: { 'line-color': '#fff', 'line-width': 3, 'line-opacity': 0.9 } })
          }
          const { LngLatBounds } = require('mapbox-gl')
          const bounds = new LngLatBounds()
          route.geometry.coordinates.forEach((c: any) => bounds.extend(c))
          mapbox.fitBounds(bounds, { padding: { top: 60, bottom: 60, left: 60, right: 60 }, maxZoom: 15 })
        }
        if (mapbox.loaded()) draw(); else mapbox.on('load', draw)
      }
    } finally {
      setLoading(false)
    }
  ...
  const canCalculate = !!(departCoords && arriveeCoords)
    ...
    <section id="estimateur" style={{ background: '#000', padding: '80px 24px', fontFamily: 'var(--font)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        >
          ...
          <p className="section-sub">Saisissez votre point de départ et destination — votre tarif s'affiche instantanément.</p>
        </motion.div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'start' }} className="est-grid">
          ...
          <div>
            ...
            {result && (
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: 16, background: '#111', borderRadius: 10, marginBottom: 16 }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#888', fontSize: '0.82rem' }}>🟢 {result.from}</span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#888', fontSize: '0.82rem' }}>🔴 {result.to}</span>
                </div>
                <a
            ...
            {!TOKEN && (
              <div style={{ marginTop: 12, padding: '12px 16px', background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: 10, color: '#555', fontSize: '0.75rem', lineHeight: 1.5 }}>
                ⚙️ Ajoutez <code style={{ background: '#111', padding: '2px 6px', borderRadius: 4, color: '#888', fontSize: '0.72rem' }}>NEXT_PUBLIC_MAPBOX_TOKEN=pk.ey...</code> dans <code style={{ background: '#111', padding: '2px 6px', borderRadius: 4, color: '#888', fontSize: '0.72rem' }}>.env.local</code> pour activer l'estimateur.
              </div>
            )}
          </div>
          ...
          <div
            style={{
              ...
              background: '#0d0d0d', border: '1px solid #1a1a1a', position: 'relative',
            }}
          >
      ...
      <style>{`
        @media (max-width: 860px) { .est-grid { grid-template-columns: 1fr !important; } }
      `}</style>
    </section>
  )
}
...
/* ─── Autocomplete field ─────────────────────────────────── */
function AutoField({
  label, value, placeholder, suggestions,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, position: 'relative' }}>
      <label style={{ color: '#888', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
        {label}
      </label>
      <input
        type="text"
      ...
      {suggestions.length > 0 && (
        <div className="est-suggestions active">
          {suggestions.map((s, i) => (
            <div key={i} className="est-sugg-item" onMouseDown={() => onSelect(s)}>
              {s.place_name}
            </div>
          ))}
        </div>
